...
function tiempo_jugado() {
  // Si el temporizador ya está activo, no crear uno nuevo
  if (timer) return;

  // Iniciar el temporizador
  timer = setInterval(() => {
    ele.innerHTML = sec1 + ':' + (sec < 10 ? '0' : '') + sec; // Formato de 2 dígitos para segundos
    sec++;
    if (sec === 60) {
      sec = 0;
      sec1++;
...
function finalizar_timer() {
  clearInterval(timer); // Detener el temporizador
  timer = null; // Reiniciar el identificador del temporizador para poder iniciarlo de nuevo en el futuro
}
